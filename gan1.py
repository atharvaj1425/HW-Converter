...
class HandwritingCharacterDataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        if len(self.image_paths) == 0:
            raise ValueError("No images found in the dataset. Check that your 'characters' folder contains correctly named image files.")
...
class Generator(nn.Module):
    def __init__(self, z_dim, n_classes, img_shape):
        ...
        # Use an embedding dimension of 1 (instead of n_classes)
        self.label_emb = nn.Embedding(n_classes, 1)
        # Determine initial spatial size; for 28x28 images, 28//4 = 7.
        self.init_size = img_shape[1] // 4  
        self.l1 = nn.Sequential(nn.Linear(z_dim + 1, 128 * self.init_size ** 2))
        self.conv_blocks = nn.Sequential(
            nn.BatchNorm2d(128),
            nn.Upsample(scale_factor=2),  # Upsample: 7 -> 14
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(128, 0.8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Upsample(scale_factor=2),  # Upsample: 14 -> 28
            nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(64, 0.8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64, img_shape[0], kernel_size=3, stride=1, padding=1),
    ...
    def forward(self, noise, labels):
        # Embed labels: shape becomes (batch, 1)
        label_input = self.label_emb(labels)
        # Concatenate noise (batch, z_dim) with label embedding (batch, 1)
        gen_input = torch.cat((noise, label_input), dim=1)
        out = self.l1(gen_input)
...
class Discriminator(nn.Module):
    def __init__(self, n_classes, img_shape):
        ...
        self.label_embedding = nn.Embedding(n_classes, 1)
        self.model = nn.Sequential(
            # Concatenate image (1 channel) + label channel = 2 channels
            nn.Conv2d(img_shape[0] + 1, 64, kernel_size=3, stride=2, padding=1),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout2d(0.25),
            nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1),
        ...
        # For a 28x28 image after downsampling layers, the feature map is assumed 2x2.
        ds_size = 2
        self.adv_layer = nn.Sequential(
            nn.Linear(512 * ds_size * ds_size + 1, 1),
            nn.Sigmoid()
    ...
    def forward(self, img, labels):
        # Embed labels: shape (batch, 1)
        label_input = self.label_embedding(labels)
        # Expand to create a label map with same spatial dimensions as image
        label_input = label_input.unsqueeze(2).unsqueeze(3)
        label_input = label_input.expand(label_input.size(0), label_input.size(1), img.size(2), img.size(3))
        # Concatenate along channels: resulting shape (batch, 1+1=2, H, W)
        d_in = torch.cat((img, label_input), dim=1)
        out = self.model(d_in)
        out = out.view(out.size(0), -1)
        # Append label embedding (1-dim) to flattened features
        out = torch.cat((out, self.label_embedding(labels)), dim=1)
        validity = self.adv_layer(out)
        return validity

# -----------------------------
# Training and Testing the Character-level cGAN (PyTorch)
# -----------------------------
def train_and_test_cgan():
    # Hyperparameters
    epochs = 50
    batch_size = 64
    lr = 0.0002
    z_dim = 100
    img_size = 28  # Assuming square images (28x28)
    channels = 1
    img_shape = (channels, img_size, img_size)
    ...
    # Update this path to your "characters" folder.
    dataset = HandwritingCharacterDataset(root_dir="characters", transform=transform)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
    ...
    # Optional: Learning rate scheduler for improved convergence
    ...
    # -----------------------------
    generator.eval()
    # For testing, choose a few labels to generate examples.
    test_labels = list(dataset.label_to_int.values())[:8]  # first 8 labels
    ...
    for idx, ax in enumerate(axes):
        ...
        ax.axis('off')
        # Map back to letter by reverse lookup
        # Here we assume the dataset mapping dictionary is invertible.
        inv_map = {v: k for k, v in dataset.label_to_int.items()}
        ax.set_title(inv_map[test_labels[idx]], fontsize=12)
    plt.tight_layout()
    plt.show()

if __name__ == "__main__":
    train_cgan_pytorch()
    # After training, you can call train_and_test_cgan() if you prefer a combined version.
